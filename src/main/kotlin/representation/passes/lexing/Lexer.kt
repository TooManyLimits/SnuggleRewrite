...
class Lexer(val fileName: String, private val scanner: Scanner): Iterable<Token> {
    ...
    fun expect(type: TokenType, extraInfo: String = ""): Token = take().let {// Expect the next token to be of the given type, if it isn't, then throw an error
        if (it?.type != type)
            throw ParsingException("$type${if (extraInfo != "") " $extraInfo" else ""}", "${it?.type ?: "End of file"}", it?.loc ?: curLoc)
        else it
    }
